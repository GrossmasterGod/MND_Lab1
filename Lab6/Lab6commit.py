...
while flag_of_model is False:
    ...
    while flag_of_dispersion is False:
        Y_matrix = np.array(
            [5.4 + 2.4 * plan_matr_for_calc_Y[:, 1] + 7.3 * plan_matr_for_calc_Y[:, 2] + 9.6 * plan_matr_for_calc_Y[:, 3] + 2.5 * plan_matr_for_calc_Y[:, 1] ** 2 +
             0.2 * plan_matr_for_calc_Y[:, 2] ** 2 + 8.2 * plan_matr_for_calc_Y[:, 3] ** 2 + 1.7 * plan_matr_for_calc_Y[:, 1] * plan_matr_for_calc_Y[:, 2] +
             0.7 * plan_matr_for_calc_Y[:, 1] * plan_matr_for_calc_Y[:, 3] + 0.6 * plan_matr_for_calc_Y[:, 2] * plan_matr_for_calc_Y[:, 3] +
             9.3 * plan_matr_for_calc_Y[:, 1] * plan_matr_for_calc_Y[:, 2] * plan_matr_for_calc_Y[:, 3] + random.randint(0, 100) - 50 for i in range(m)]).T
        mean_Y = np.mean(Y_matrix, axis=1)
